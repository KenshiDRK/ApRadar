...
#Region "Identity"
    ...
    End Sub
    Public Sub New(ByVal professionalColorTable As ProfessionalColorTable)
...
#Region "OnRenderButtonBackground"
    ...
    Protected Overloads Overrides Sub OnRenderButtonBackground(ByVal e As ToolStripItemRenderEventArgs)
        ...
        If button.BackgroundImage Is Nothing Then
            ...
            Else
                If button.Tag = "Item" Then
                    DrawButtonBorder(e.Graphics, New Rectangle(Nothing, button.Bounds.Size))
                End If
            End If
        Else
    ...
    Protected Overloads Overrides Sub OnRenderItemText(ByVal e As ToolStripItemTextRenderEventArgs)
        If (TypeOf e.ToolStrip Is MenuStrip) OrElse (TypeOf e.ToolStrip Is ToolStrip) OrElse (TypeOf e.ToolStrip Is ContextMenuStrip) OrElse (TypeOf e.ToolStrip Is ToolStripDropDownMenu) Then
            ...
            Else
                ...
                Else
                    If TypeOf e.Item Is ToolStripLabel OrElse TypeOf e.Item Is ToolStripButton OrElse TypeOf e.Item Is ToolStripDropDownButton Then
                        If TypeOf e.Item Is ToolStripButton Then
                            If CType(e.Item, ToolStripButton).Selected OrElse CType(e.Item, ToolStripButton).Pressed Then
                                e.TextColor = e.Item.ForeColor  '_textContextMenuItem
                            Else
            ...
            Using clearTypeGridFit As New UseClearTypeGridFit(e.Graphics)
                'If TypeOf e.Item Is ToolStripLabel Then
                '    RenderCustomItemText(e)
                'Else
...
#End Region
    ...
    Private Sub RenderCustomItemText(ByVal e As ToolStripItemTextRenderEventArgs)
        Dim g As Graphics = e.ToolStrip.CreateGraphics
        g.TextRenderingHint = TextRenderingHint.AntiAlias
        Dim mx As New Matrix(0.25F, 0, 0, 0.25F, 1, 1)
        g.Transform = mx
        g.DrawString(e.Text, e.TextFont, Brushes.Black, e.TextRectangle)
        g.Dispose()
        e.Graphics.InterpolationMode = InterpolationMode.HighQualityBicubic
        e.Graphics.TextRenderingHint = TextRenderingHint.AntiAlias
        e.Graphics.DrawString(e.Text, e.TextFont, Brushes.White, e.TextRectangle)
    End Sub
    ...
    Private Sub DrawGradientToolItem(ByVal g As Graphics, ByVal item As ToolStripItem, ByVal colors As GradientItemColors)
        ...
        If item.Tag = "Item" Then
            DrawGradientItem1(g, New Rectangle(Point.Empty, item.Bounds.Size), colors)
        Else
    ...
    Private Sub DrawGradientItem1(ByVal g As Graphics, ByVal backRect As Rectangle, ByVal colors As GradientItemColors)
        ' Cannot paint a zero sized area
        If (backRect.Width > 0) AndAlso (backRect.Height > 0) Then
            ' Draw the background of the entire item
            DrawGradientBack1(g, backRect, colors)

            ' Draw the border of the entire item
            DrawButtonBorder(g, backRect)
        End If
    End Sub
    ...
    Private Sub DrawGradientBack1(ByVal g As Graphics, ByVal backRect As Rectangle, ByVal colors As GradientItemColors)
        ' Reduce rect draw drawing inside the border
        backRect.Inflate(-1, -1)
        ...
        Using fillBrush2 As New LinearGradientBrush(backRect2, Color.FromArgb(0, Color.White), Color.FromArgb(72, Color.SteelBlue), 90.0F)
            ' Reduce rect one more time for the innermost drawing
            'backRect.Inflate(-1, -1)
    ...
    Private Sub DrawButtonBorder(ByVal g As Graphics, ByVal backRect As Rectangle)
        'Using uaa As New UseAntiAlias(g)
        '    Dim backRectI As Rectangle = backRect
        '    backRectI.Inflate(-1, -1)
        '    Using borderPath As GraphicsPath = CreateBorderPath(backRectI, _cutMenuItemBack)
        '        g.DrawPath(Pens.Gray, borderPath)
        '    End Using
        '    backRectI.Inflate(1, 1)
        '    Using borderPath As GraphicsPath = CreateBorderPath(backRectI, _cutMenuItemBack)
    ...
    Protected Overloads Sub DrawSeparator(ByVal g As Graphics, ByVal vertical As Boolean, ByVal rect As Rectangle, ByVal lightPen As Pen, ByVal darkPen As Pen, ByVal horizontalInset As Integer, _
    ...
    Private Function CreateHighlightPath(ByVal rect As Rectangle, ByVal cut As Single) As GraphicsPath
        Dim gp As New GraphicsPath
        Dim p As New List(Of PointF)
        p.Add(New PointF(rect.Left + cut, rect.Bottom - cut))
        p.Add(New PointF(rect.Left + rect.Width / 2, rect.Height))
        p.Add(New PointF(rect.Right - cut, rect.Bottom - cut))
        gp.AddLine(rect.Left + cut, rect.Bottom, rect.Right - cut, rect.Bottom)
        gp.AddCurve(p.ToArray)
        Return gp
    End Function
