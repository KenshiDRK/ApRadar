...
#Region " MEMBER VARIABLES "
    ...
    Private WithEvents MemoryScanTimer As New System.Windows.Forms.Timer
    Private MousePosTimer As New System.Windows.Forms.Timer
    ...
    Private _outlineBrush As New SolidBrush(Color.FromArgb(15, Color.Pink))
...
#Region " RADAR METHODS "
    ...
    ''' <remarks></remarks>
    Public Sub RenderRadar(ByVal g As Graphics)
        Try
            ...
            CorePaintData.CenterPoint = New PointF(CSng(_parent.Width) / 2.0F, CSng(_parent.Height) / 2.0F)

            'Set the scale
            If RadarType = RadarTypes.Mapped Then
                'Get the x and y scale
                CorePaintData.XScale = CSng(_parent.Width) / 512.0F * Settings.Zoom
                CorePaintData.YScale = CSng(_parent.Width) / 512.0F * Settings.Zoom
                'Get the map scale values
                CorePaintData.MapScaleX = CSng(_parent.Width) / 512.0F
                CorePaintData.MapScaleY = CSng(_parent.Height) / 512.0F
                PaintMappedRadar(g)
            Else
                CorePaintData.XScale = CSng(_parent.Width) / 50.0F
                CorePaintData.YScale = CSng(_parent.Height) / 50.0F
                PaintOverlayRadar(g)
            End If
...
#Region " MAPPED RADAR PAINT METHODS "
    ...
    Private Sub PaintMappedRadar(ByVal g As Graphics)
        Try
            ...
            If CurrentMapEntry Is Nothing Then
                ...
                CorePaintData.XScale = _parent.Width / 50
                CorePaintData.YScale = _parent.Height / 50
        ...
        If Not MapImage Is Nothing Then
            If Settings.Zoom = 1 Then
                g.DrawImage(MapImage, _
                            New RectangleF(0.0F, 0.0F, _parent.Width, _parent.Height), _
    ...
    Private Sub PaintMiniMap(ByVal g As Graphics, ByVal XOffset As Single, ByVal YOffset As Single, ByVal BoxSize As Single)
        If Not MapImage Is Nothing Then
            ...
            mmWidth = _parent.Width / 3
            mmHeight = _parent.Height / 3
            XOffset = XOffset * (mmWidth / _parent.Width) + (mmWidth / 4)
            YOffset = YOffset * (mmHeight / _parent.Height) + (mmHeight / 4)
            BoxSize = BoxSize * (mmWidth / _parent.Width)
            g.DrawImage(MapImage, _
                        New RectangleF(5.0F, 5.0F, (_parent.Width / 3), _parent.Height / 3), _
                        New RectangleF(0.0F, 0.0F, 512.0F, 512.0F), _
                        GraphicsUnit.Pixel)
...
#Region " OVERLAY RADAR PAINT METHODS "
    ...
    Private Sub PaintMobData(ByVal g As Graphics, ByVal Target As MobData, ByVal ClaimedBy As Integer, ByVal IsTarget As Boolean)
        ...
        Try
            ...
            If Not mob Is Nothing Then
                ...
                size.Width += 10
                size.Height += 30
                g.FillRectangle(New SolidBrush(Color.FromArgb(128, 43, 56, 63)), New Rectangle(New Point(25, 390), size.ToSize))
                size.Width += 1
                size.Height += 1
                g.DrawRectangle(New Pen(Color.FromArgb(128, Color.Gray), 2), New Rectangle(New Point(25, 390), size.ToSize))
                ...
                g.DrawString(_mobInfo, Settings.DataFont, New SolidBrush(Color.FromArgb(128, Color.White)), 29, 394)
            Else
    ...
    Private Sub PaintMobTracker(ByVal g As Graphics)
        If Not _trackMob Is Nothing Then
            ...
            Dim centerPoint As New PointF(_parent.Width / 2, 150)
...
#Region " SHARED PAINT METHODS "
    ...
    Private Sub PaintMob(ByVal g As Graphics, ByVal Mob As MobData)
        ...
        '            If Mob.ID = _targetId Then
        '                DrawingHelpers.DrawOutlineString(g, _displayText, Me.Font, _outlineBrush, Mob.MapX + Settings.BlipSize, Mob.MapY - (Settings.BlipSize / 2) - 1, 4)
        '            End If
...
#Region " PUBLIC METHODS "
    ...
    Public Sub InitializeRadar(ByVal Parent As Form)
...
#Region " THREADS "
    ...
    Private Sub OnScanCompleted()
        _sync.Post(AddressOf RaiseScanCompleted, Nothing)
    End Sub

    Private Sub RaiseScanCompleted()
        RaiseEvent ScanCompleteted()
    End Sub
